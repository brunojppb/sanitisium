...
RUN cargo zigbuild -r \
  --target x86_64-unknown-linux-musl --target aarch64-unknown-linux-musl && \
  mkdir /app/linux && \
  cp target/aarch64-unknown-linux-musl/release/web-server /app/linux/arm64 && \
  cp target/x86_64-unknown-linux-musl/release/web-server /app/linux/amd64
...
COPY --from=builder /app/${TARGETPLATFORM} /usr/bin/web-server
