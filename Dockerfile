...
# To make sanitisium compatible with different linux distributions,
# we cross-compile using musl so the binary is statically linked with the right dependencies
...
RUN rustup show \
  ...
  && rustc --version
RUN cargo build --verbose --release --package web-server
